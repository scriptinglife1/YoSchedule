...
public class CalendarQuickstart {
    ...
    public void addLectureUsingBatch(LinkedList<Lecture> lectures) throws IOException {
        ...
        for (Lecture lecture: lectures){
            ...
            service.events().insert(createdCalendar.getId(),lectureEvent).queue(batch,callback);
        }
        ...
        do {
            ...
            for (CalendarListEntry calendarListEntry : items) {
                if(calendarListEntry.getSummary().equals("Calendar for YoSchedule")){
                    ...
                    createdCalendar = service.calendars().get(calendarListEntry.getId()).execute();
...
// creating calendar if there are no calendars with same description
    ...
    public static void main(String[] args) throws IOException {
        ...
        // List the next 10 events from the primary calendar.
        DateTime now = new DateTime(System.currentTimeMillis());
        Events events = service.events().list(createdCalendar.getId())
            .setMaxResults(10)
            .setTimeMin(now)
            .setOrderBy("startTime")
            .setSingleEvents(true)
    ...
    public static void checkDateTime(String eventTime) {
        ...
        if (eventTime.contains(currentDate)) {
            int number1 = ((Character.getNumericValue(eventTime.charAt(11)) * 10 +( Character.getNumericValue(eventTime.charAt(12))) +1) * 60) +
